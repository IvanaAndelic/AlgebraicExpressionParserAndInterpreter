...
{
    [TestClass]
    public class TestPowerExpression
    {
        [TestMethod]
        public void PowerExpressionForTwoConstantsReturnsFirstConstantToThePowerOfSecond()
        {
            ...
            PowerExpression power = new PowerExpression(@base, exponent);

            Context context = new Context(5);
            Assert.AreEqual(8, power.Interpret(context), 1e-10);

            Assert.AreEqual(-8, new PowerExpression(new Constant(-2), exponent).Interpret(context), 1e-10);
        }
        ...
        public void PowerExpressionForTwoVariablesReturnsVariableToThePowerOfVariable()
        {
            ...
            PowerExpression power = new PowerExpression(var, var);
